...
export const chatService = {
  ...
  processPatientInfo: async (patientInfo: any) => {
    const response = await axios.post(`${API_URL}/patient`, { patientInfo });
  ...
  sendMessage: async (patientId: string, message: string) => {
    const response = await axios.post(`${API_URL}/patient/${patientId}/chat`, { message });
    return response.data;
  },
  ...
  updatePatient: async (patientId: string, patientInfo: any) => {
    const response = await axios.put(`${API_URL}/patient/${patientId}`, { patientInfo });
    return response.data;
  }
}; 